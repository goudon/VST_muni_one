...
## ウィンドウ
- 初期サイズ: **360 × 320 px** (耳の最大回転 + ノートオン パルス拡大でも見切れないサイズ)
- リサイズ: 不可

## 描画領域の根拠
- 顔ベース: 208 × 196 px (ネイティブ)
- 耳の回転 (±0.5 rad): pivot は耳画像の上端付近 (`center.y - h*0.4`)。最大回転時、
  耳 bbox は顔ベース端からさらに **横 +52 px / 縦 +10 px** ほど膨らむ。
- ノートオン パルス: 顔ベース中央を中心に **+8% スケール**。耳の最遠端がさらに
  約 +8 px 外側に押し出される。
- 上部バー: margin 10 + ボタン高 32 + spacing 4 = 46 px
- → 顔ベース左右に最低 60 px ずつの余白を確保 → MuniFace local 幅 ≥ 328、
  ウィンドウ幅 = 328 + margin 20 = 348 → 余裕を見て **360**。
- 縦方向は 上部 46 + 顔ベース 196 + パルス時の下方拡張 10 + 余白 ≈ 270 → **320** で十分。
